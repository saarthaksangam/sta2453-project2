...
sidebar <- dashboardSidebar(disable=FALSE,width='190px',
                            ...
                            ########
                            sidebarMenu(id = 'tabs', 
                                        selectInput("partition", "Country", 
                                                    choices = c("Canada", "Japan","United Kingdom"),
                                                    selected = "Canada"),
                                        menuItem("Data", icon = icon("line-chart"), tabName = "time"),
                                        menuItem("Model", icon = icon("list"), tabName = "nphet"),
...
body <- dashboardBody(
    ...
    # title
    titlePanel("Covid-19 Dashboard for Canada, Japan and UK"),
    ...
    tabItems(
        ...
        # Time series Tab #
        tabItem("time", 
                h3(paste0("Date: 2020-02-04 to ", daily_date)
                  ),
                h3(paste0("Data Source:"), tags$a(href="https://cloud.r-project.org/web/packages/COVID19/index.html", 
                                                 "COVID19", target="_blank"))
                ,
                ...
                fluidRow(
                  ...
                  tabPanel("Daily data", dataTableOutput("dattable"))
                    )
                  ),
        ...
        tabItem("nphet",
                ...
                fluidRow(
                  tabBox(title = "County data",
                    ...
                    tabPanel("Incidence table",
                             dataTableOutput("countytable", width = "70%")),
                ...
                #              column(10, dataTableOutput("weekarea"))
                # )),
                fluidRow(
...
server <- function(input, output) {
    ...
    dataRaw <- reactive({
      ...
      if(input$partition == "Canada") return(data_Canada)
      if(input$partition == "Japan") return(data_Japan)
      if(input$partition == "United Kingdom") return(data_UK)
    })
    ...
    output$recentplot <- renderPlotly({
      ...
      g = dataRaw() %>%
        ...
        filter(date > as.Date(daily_date,format="%Y-%m-%d")-input$recent) %>%
    ...
    ## Table county data
    output$weekarea <- renderDataTable({
