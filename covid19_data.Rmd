          ...
          div(
            dropdownButton(
                ...
                dateInput("forecasting_start",
                  ...
                  min = min(all_data$date),
                  max = max(all_data$date),
                  value = as.Date(max(all_data$date))-14),
...
server <- function(input, output) {
    ...
    output$forecasting <- renderPlot({
        ...
        plot = data %>%
            ggplot(aes(x=date, y=daily_cases_smoothed)) + 
            geom_line(size=1.5) +
            geom_line(aes(y=forecasting), color = "red", size=1.5, filter(data, date < Re_end_date & date > Re_start_date)) +
    ...
    output$forecast_box <- renderUI({
        ...
        list(
          ...
          infoBox(
          "Difference cases", paste0(" ", actual_number - forecast_number), icon = shiny::icon("bar-chart"),
